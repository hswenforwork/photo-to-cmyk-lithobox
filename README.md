...
離線可用的純前端網頁工具：上傳一張生活照片，輸出3D列印彩色燈箱需要的全部檔案——灰階高度圖（3D列印浮雕用）、線稿（熄燈裝飾層用，提供「簡單線稿」與「素描風」兩種風格可切換）、C/M/Y三張調色透明片，以及3D列印用的STL模型。內建AI主體偵測（本機執行，人物／動物輪廓辨識），照片全程不會上傳到任何伺服器。

**線稿有兩種風格可選：**
- **簡單線稿**：邊緣偵測黑白描邊，輸出PNG＋SVG（SVG可直接匯入雷射雕刻軟體）。
- **素描風**：模擬鉛筆素描的濃淡漸層與交叉排線筆觸，輸出一般PNG＋雷射雕刻專用灰階點陣PNG（目前為第一階段的純程式化濾鏡效果，非AI模型）。
兩種風格都只會套用在「主體遮罩」範圍內（忽略背景），且即時合成預覽（點燈／熄燈）會自動跟著目前選擇的風格切換。打包下載ZIP時可勾選是否同時包含另一種風格的檔案。
...
- 素描風線稿目前是第一階段的純程式化濾鏡（負片＋模糊＋加亮混合明暗骨架＋固定角度交叉排線），效果較保守、規律，尚非手繪等級的逼真筆觸；若日後需要更接近真實素描的效果，可考慮升級成AI圖像轉素描模型（類似目前AI主體偵測的整合方式）。
- 素描風的雷射雕刻灰階PNG，其「深淺對應雕刻深度」的編碼慣例依常見雷射雕刻軟體設計（深色=雕更深），如果與您實際使用的軟體相反，可在下載前勾選「反轉黑白」。
