...
**外層裝飾層有兩種風格可選：**
- **日式似顏繪風格**：以本機執行的AI圖像風格轉換模型（CartoonGAN）將照片轉換為帶有簡單色彩、俐落墨線的日式動畫風全彩插畫，可選宮崎駿吉卜力風／細田守風／今敏風／新海誠風四種畫風。
...
- 日式似顏繪風格為圖像風格轉換（CartoonGAN），效果會依照片內容、光線與構圖而異，並非傳統五官誇張變形的手繪似顏繪；運算成本較高，會依序嘗試WebGL（含偵測軟體模擬WebGL並自動改用WASM）→WASM→CPU運算，沒有真實GPU加速的裝置速度會較慢，因此即時預覽需手動按「產生／更新預覽」按鈕觸發，不會隨裁切/縮放即時自動更新。
