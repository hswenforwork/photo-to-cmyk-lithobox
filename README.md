...
兩種風格皆只會套用在「主體遮罩」範圍內（忽略背景），且即時合成預覽（點燈／熄燈）會自動跟著目前選擇的風格切換。所有PNG列印圖層皆已排版置中於A4紙並預留5mm列印邊界，各圖層比例一致、可直接家用印表機列印；另提供雷射雕刻專用灰階PNG。打包下載ZIP時可勾選是否同時包含另一種風格的檔案。
...
- 日式似顏繪風格為圖像風格轉換（CartoonGAN），效果會依照片內容、光線與構圖而異，並非傳統五官誇張變形的手繪似顏繪；運算成本較高，沒有獨立顯示卡（WebGL）的裝置會自動改用WASM/CPU運算，速度較慢。
