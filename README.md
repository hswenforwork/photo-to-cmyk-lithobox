...
- 日式似顏繪風格為圖像風格轉換（CartoonGAN），效果會依照片內容、光線與構圖而異，並非傳統五官誇張變形的手繪似顏繪；運算成本較高，會依序嘗試WebGL（含偵測軟體模擬WebGL並自動改用WASM）→WASM→CPU運算，沒有真實GPU加速的裝置速度會較慢，因此即時預覽需手動按「產生／更新預覽」按鈕觸發，不會隨裁切/縮放即時自動更新。長時間使用WebGL backend偶爾會因內部貼圖管理累積而觸發「貼圖尺寸超過上限」錯誤，已加入自動改用WASM backend重試一次的容錯機制。
